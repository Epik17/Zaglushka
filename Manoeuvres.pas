...
function Virage(helicopter : THelicopter; initialstate : TStateVector; icG, icT,kren,tempVvodaVKren{градусы}, deltaPsi{градусы}: Real) : TManevrData;
function Spiral(helicopter : THelicopter; initialstate : TStateVector; icG, icT,kren,tempVvodaVKren{градусы}, deltaPsi{градусы}, Vy{m/s}: Real) : TManevrData;
...
function ForcedVirage(helicopter : THelicopter; initialstate : TStateVector; icG, icT,kren,tempVvodaVKren{градусы}, deltaPsi{градусы}: Real) : TManevrData;
...
function iVirageSpiral(helicopter : THelicopter; initialstate : TStateVector; icG, icT,kren{градусы}, tempVvodaVKren{градусы}, deltaPsi{градусы},Vy {m/s}: Real; Left, Forced: Boolean) : TManevrData;
var
...
begin
   ...
   if not constKren then
    begin
     omega.x:=DegToRad(tempVvodaVKren);  //скорость ввода в крен (вывода из крена), градусов в секунду
...
function iiVirage(helicopter : THelicopter; initialstate : TStateVector; icG, icT,kren, tempVvodaVKren{градусы},deltaPsi{градусы}: Real; Forced : Boolean) : TManevrData;
begin
  if deltaPsi < 0 then
    Result:=iVirageSpiral(helicopter, initialstate, icG, icT,kren, tempVvodaVKren{градусы},deltaPsi, 0, False, Forced);

  if deltaPsi > 0 then
    Result:=iVirageSpiral(helicopter, initialstate, icG, icT,kren, tempVvodaVKren{градусы},deltaPsi, 0, True, Forced);
...
function Virage(helicopter : THelicopter; initialstate : TStateVector; icG, icT,kren,tempVvodaVKren{градусы}, deltaPsi{градусы}: Real) : TManevrData;
begin
  Result := iiVirage(helicopter, initialstate, icG, icT,kren, tempVvodaVKren{градусы},deltaPsi{градусы}, False)
end;

function ForcedVirage(helicopter : THelicopter; initialstate : TStateVector; icG, icT,kren, tempVvodaVKren{градусы},deltaPsi{градусы}: Real) : TManevrData;
begin
   Result := iiVirage(helicopter, initialstate, icG, icT,kren, tempVvodaVKren{градусы},deltaPsi{градусы}, True)
end;

function Spiral(helicopter : THelicopter; initialstate : TStateVector; icG, icT,kren, tempVvodaVKren{градусы},deltaPsi{градусы}, Vy{m/s}: Real) : TManevrData;
begin
  if deltaPsi < 0 then
    Result:=iVirageSpiral(helicopter, initialstate, icG, icT,kren, tempVvodaVKren{градусы},deltaPsi, Vy, False, False);
  if deltaPsi > 0 then
    Result:=iVirageSpiral(helicopter, initialstate, icG, icT,kren, tempVvodaVKren{градусы},deltaPsi, Vy, True, False);
  if deltaPsi = 0 then
   SetLength(Result,0)
end;
  ...
  if (Vtemp>=Vmin) and (Vtemp<=Vmax) then
   ...
   begin
     if thetaSlope < 0 then
      manevr := 'снижение по наклонной'
     else
      manevr := 'набор высоты по наклонной';
...
function iRazvorotNaGorke (helicopter : THelicopter; initialstate : TStateVector; icG, icT,nyvvoda,nyvyvoda,thetaSlope,Vvyvoda,kren,dkurs : Real) : TManevrData;
var
 vvod,nakl,vvodvkren,stable, vyvod : TManevrData;
         ...
         repeat
          if (tempstate.V > 0) then
           begin
            ...
            SetOmegaAndAccelerationBR(tempomega, tempa, tempstate,nyTemp,nx(helicopter, nyTemp, icG, icT,tempstate.y,g_mps*tempstate.V),dnxa,nxOtXvr(helicopter,tempstate.y,icG,g_mps*tempstate.V),RadToDeg(nakl[High(nakl)].gamma),kren, localTime, gammaDot); //переводим скорость в км/ч
            g_Etape(vvodvkren,tempstate, helicopter, nyTemp,tempa, tempomega);
            HmaxCheck(tempstate, failed);
           end
          else
          failed := True;
         until
            ((Abs(RadToDeg(tempstate.gamma))>=Abs(kren)) and (not failed));
 ...
 //стыкуем
  if not failed then
    begin
     ...
     AppendManevrData(Result,vvod,helicopter);
     AppendManevrData(Result,nakl,helicopter);
     AppendManevrData(Result,vvodvkren,helicopter);
     AppendManevrData(Result,stable,helicopter);
     AppendManevrData(Result,vyvod,helicopter);
    end
  else
         ...
         repeat
          if (tempstate.V > 0) then
           ...
           // nyTemp := Cos(tempstate.theta) / Cos(tempstate.gamma);
            ...
            SetOmegaAndAccelerationBR(tempomega, tempa, tempstate,nyslope,nx(helicopter, nyslope, icG, icT,tempstate.y,g_mps*tempstate.V),dnxa,nxOtXvr(helicopter,tempstate.y,icG,g_mps*tempstate.V),0,kren, localTime, c_gammaDot); //переводим скорость в км/ч
            g_Etape(razvorot,tempstate, helicopter, nyslope,tempa, tempomega);
            HmaxCheck(tempstate, failed);
           end
          else
          failed := True;
